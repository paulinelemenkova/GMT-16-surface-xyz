...
# Check up dimensions of the table (data range)
gmt info topo_MT.xyz
# output: topo_MT.xyz: N = 3815189    <120.0083/160.0083>    <5.002/30.0019>    <-10913/3559>

# Generate 1 by 1 minute block mean values from the raw ASCII data (xyg table)
...
# Make gradient illumination with azimuth 45 degree
gmt grdgradient Surface_MT.nc -GSurface_MT.int -A0/45 -Ne1 -fg

# Generate a color palette table from grid
# gmt makecpt --help
gmt makecpt -Cseis -T-11000/4000 > colors.cpt
#gmt makecpt -Cviridis -E20 > colors.cpt
# -Ic inverse palette
...
# Make raster image
gmt grdimage Surface_MT.nc -Ccolors -R120/160/5/30 -JM6i -P -ISurface_MT.int -Xc -K > $ps

# Add grid
gmt psbasemap -R -J \
    --MAP_FRAME_AXES=wESN \
    --MAP_TITLE_OFFSET=1.4c \
    -Bpxg6f2a4 -Bpyg6f1a4 -Bsxg2 -Bsyg2 \
...
gmt psscale -R -J -Ccolors.cpt \
    -Dg120.5/4+w10.0c/0.4c+v+o-1.8/0.2c+ml  \
    --FONT_LABEL=7p,Helvetica,dimgray \
    --FONT_ANNOT_PRIMARY=5p,Helvetica,dimgray \
    -Ba2000g1000f100+l"Surface color model scale: seis" \
    -I0.2 -By+lm -O -K >> $ps
